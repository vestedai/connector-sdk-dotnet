...
The .NET SDK sets this via gRPC `Metadata`:

```csharp
// Internally generated by GrpcClient; shown for protocol illustration only.
...
`connector_audit_events`. The .NET SDK surfaces this as a `TokenException`.
...
token before the old token's revocation TTL expires. The .NET SDK supervisor
