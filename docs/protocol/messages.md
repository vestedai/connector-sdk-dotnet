...
```proto
message Hello {
  string sdk_language = 1;  // "node", "python", "php", "dotnet", …
...
|---|---|---|---|
| `sdk_language` | string | yes | Language identifier; used in admin UI and audit log. The .NET SDK sends `"dotnet"`. |
...
.NET SDK example — the SDK constructs this automatically:
...
{
    SdkLanguage = "dotnet",
    SdkVersion  = VestedAI.ConnectorSdk.Version.SdkVersion,
...
| `sensitivity` | string | Optional. One of `"read"`, `"write"`, `"destructive"`, `"external_call"`, `"medium"`. Empty → hub defaults to `"external_call"`. |

The .NET SDK auto-generates `input_schema_json` from the `TArgs` POCO via NJsonSchema, serialized to UTF-8 bytes.
